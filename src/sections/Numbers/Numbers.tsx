...
const Numbers = () => {
    ...
    return (
        <section className="flex flex-wrap w-[90vw] justify-center gap-5 m-auto my-10">
            {cards.map((card) => (
                <div key={card.id} className="bg-primary rounded-xl size-72 flex flex-col items-center pt-2 gap-5">
                    <h3 className="text-pop text-4xl font-semibold">{card.number}</h3>
                    <p className="text-oposite text-lg my-auto px-2 text-center leading-normal">{card.text}</p>
                </div>
            ))}
        </section>
    )
